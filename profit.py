...
# ITEMS #
#########
COPPER_WIRE = Sellable("Copper Wire", 10_000, [(COPPER_BAR, 2)], smelt_time=14)
IRON_NAILS = Sellable("Iron Nails", 20_000, [(IRON_BAR, 2)], smelt_time=29)
BATTERY = Sellable("Battery", 70_000, [(COPPER_WIRE, 1), (COPPER_BAR, 4)], smelt_time=58)
HAMMER = Sellable("Hammer", 135_000, [(IRON_NAILS, 1), (LEAD_BAR, 2)], smelt_time=1*60 + 56)
GLASS = Sellable("Glass", 220_000, [(SILICON_BAR, 4)], smelt_time=2*60+55)
CIRCUIT = Sellable("Circuit", 620_000, [(SILICON_BAR, 2), (ALUMINUM_BAR, 2), (COPPER_WIRE, 4)], smelt_time=4*60+52)
LENS = Sellable("Lens", 1_100_000, [(GLASS, 1), (SILVER_BAR, 2)], smelt_time=9*60+44)
LASER = Sellable("Laser", 3_200_000, [(GOLD_BAR, 2), (LENS, 1), (IRON_BAR, 4)], smelt_time=14*60+37)
BASIC_COMPUTER = Sellable("Basic Computer", 7_600_000, [(CIRCUIT, 2), (SILVER_BAR, 2)], smelt_time=19*60+29)
SOLAR_PANEL = Sellable("Solar Panel", 12_500_000, [(CIRCUIT, 2), (GLASS, 4)], smelt_time=24*60+22)
LASER_TORCH = Sellable("Laser Torch", 31_000_000, [(BRONZE_BAR, 2), (LASER, 1), (LENS, 2)], smelt_time=29*60+14)
ADVANCED_BATTERY = Sellable("Advanced Battery", 35_000_000, [(STEEL_BAR, 9), (BATTERY, 13)], smelt_time=36*60+33)
THERMAL_SCANNER = Sellable("Thermal Scanner", 71_500_000, [(PLATINUM_BAR, 2), (LASER, 1), (GLASS, 2)], smelt_time=43*60+51)
ADVANCED_COMPUTER = Sellable("Advanced Computer", 180_000_000, [(TITANIUM_BAR, 2), (BASIC_COMPUTER, 2)], smelt_time=51*60+10)
NAVIGATION_MODULE = Sellable("Navigation Module", 1_000_000_000, [(THERMAL_SCANNER, 1), (LASER_TORCH, 1)], smelt_time=54*60+49)
PLASMA_TORCH = Sellable("Plasma Torch", 1_150_000_000, [(IRIDIUM_BAR, 6), (LASER_TORCH, 2)], smelt_time=60*60)
RADIO_TOWER = Sellable("Radio Tower", 1_450_000_000, [(PLATINUM_BAR, 32), (ALUMINUM_BAR, 65), (TITANIUM_BAR, 22)], smelt_time=60*60+3*60)
